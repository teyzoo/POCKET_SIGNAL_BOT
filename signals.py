...
quality: float
# Время открытия следующей M1-свечи —
# фактическое плановое время входа.
entry_time: datetime
...
def analyze(
    ...
    if not candles:
        return None
    # ----------------------------------------------------
    # Фиксируем последнюю полностью закрытую M1-свечу.
    # ----------------------------------------------------
    ...
    # ----------------------------------------------------
    # Время входа = начало следующей M1-свечи.
    #
    # Например:
    #
    ...
    if probability < min_probability:
        return None
    # ----------------------------------------------------
    # Время закрытия считается от точного времени входа,
    # а не от момента выполнения Python-кода.
    # ----------------------------------------------------
    close_time = (
        entry_time
        + timedelta(
